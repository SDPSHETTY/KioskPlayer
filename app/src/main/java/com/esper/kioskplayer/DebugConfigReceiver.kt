...
class DebugConfigReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        values["stream_url"] = intent.getStringExtra("stream_url")
        values["source_preference"] = intent.getStringExtra("source_preference")
        values["schedule_start"] = intent.getStringExtra("schedule_start")
        values["schedule_end"] = intent.getStringExtra("schedule_end")
        values["schedule_days"] = intent.getStringExtra("schedule_days")
        ...
        if (intent.hasExtra("schedule_enabled")) values["schedule_enabled"] = intent.getBooleanExtra("schedule_enabled", false)
        if (intent.hasExtra("fallback_on_stream_error")) values["fallback_on_stream_error"] = intent.getBooleanExtra("fallback_on_stream_error", true)
        ...
        if (intent.hasExtra("image_duration_sec")) values["image_duration_sec"] = intent.getIntExtra("image_duration_sec", 10)
        if (intent.hasExtra("heartbeat_sec")) values["heartbeat_sec"] = intent.getIntExtra("heartbeat_sec", 60)
